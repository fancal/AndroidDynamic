 ...
 *
 * @author renyugang
 * @see {@link DLBasePluginAppCompatActivity#that}
 */
public class DLBasePluginAppCompatActivity extends AppCompatActivity implements DLPlugin {
    ...
    /**
     * 虚拟出来的连接方法,在真实的activity生命周期中并不存在,代理模式的时候被作为创建的入口
     * */
    @Override
    ...
    public void onBackPressed() {
        if (mFrom == DLConstants.FROM_INTERNAL) {
            ...
            if (mProxyActivity instanceof DLProxyAppCompatActivity) {
                ((DLProxyAppCompatActivity) mProxyActivity).onSuperBackPressed();
            }
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (mFrom == DLConstants.FROM_INTERNAL) {
            ...
            if (mProxyActivity instanceof DLProxyAppCompatActivity) {
                return ((DLProxyAppCompatActivity) mProxyActivity).onSuperTouchEvent(event);
            }
        }
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        if (mFrom == DLConstants.FROM_INTERNAL) {
            ...
            if (mProxyActivity instanceof DLProxyAppCompatActivity) {
                return ((DLProxyAppCompatActivity) mProxyActivity).onSuperKeyUp(keyCode, event);
            }
        }
    ...
    public android.support.v7.app.ActionBar getSupportActionBar() {
        ...
        return mProxyActivity.getSupportActionBar();
    }
    ...
    public void setSupportActionBar(@Nullable Toolbar toolbar) {
        if (mFrom == DLConstants.FROM_INTERNAL) {
            ...
            mProxyActivity.setSupportActionBar(toolbar);
        }
    }
}
